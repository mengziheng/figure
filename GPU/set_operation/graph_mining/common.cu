...
tuple<int *, int *, int *, int> loadGraphWithName(string Infilename, string pattern)
{
    ...
    for (int i = 0; i < vertex_count; i++)
        sort(adjcant + degree_offset[i], adjcant + degree_offset[i + 1], hash_compare);
    ...
    for (int i = 0; i < vertex_count; i++)
    {
        if (degree_offset[i + 1] - degree_offset[i] < 1000)
