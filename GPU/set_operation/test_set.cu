...
#define MIN_BUCKET_NUM 2
#define MIN_BUCKET_NUM_2 8
#define map_value(hash_value, bucket_num) (((hash_value) % MIN_BUCKET_NUM) * (bucket_num / MIN_BUCKET_NUM) + (hash_value) / MIN_BUCKET_NUM)
// #define map_value_opt(hash_value, bucket_num) (((hash_value) % MIN_BUCKET_NUM) * (bucket_num / MIN_BUCKET_NUM) + (hash_value / MIN_BUCKET_NUM) % MIN_BUCKET_NUM) // 加法后面的部分是第二个哈希函数
...
bool hash_compare_opt(int a, int b)
{
  if (a % MIN_BUCKET_NUM < b % MIN_BUCKET_NUM)
    return true;
  if (a % MIN_BUCKET_NUM == b % MIN_BUCKET_NUM)
    return ((a / MIN_BUCKET_NUM) % MIN_BUCKET_NUM_2) < ((b / MIN_BUCKET_NUM) % MIN_BUCKET_NUM_2);
  return false; // 升序排列
}

// 按列存储
HT build_hash(int *array, int size, int opt = false)
{
  int *hash_table;
  ...
  for (int i = 0; i < size; i++)
  {
    ...
    if (opt == 2)
    {
      int temp = (item / MIN_BUCKET_NUM) % (bucket_num / MIN_BUCKET_NUM);
      bucket_id = ((bucket_id) % MIN_BUCKET_NUM) * (bucket_num / MIN_BUCKET_NUM) + (temp % MIN_BUCKET_NUM_2) * (bucket_num / MIN_BUCKET_NUM / MIN_BUCKET_NUM_2) + temp / MIN_BUCKET_NUM_2;
    }
    int index = 0;
...
__device__ bool search_hash(int tid, int item, int *hash_table, int hash_table_size, int opt, bool show)
{
...
__global__ void intersection_kernel(int *probe_array, int probe_array_size, int *hash_table, int hash_table_size, int *output, int opt, bool show)
{
  ...
  for (int i = tid; i < probe_array_size; i = i + blockDim.x * gridDim.x)
  {
    ...
    bool exist = true;
    exist = search_hash(tid, item, hash_table, hash_table_size, opt, show);
    if (exist)
      output[i] = item;
    else
      output[i] = -1;
...
std::tuple<int, int *> intersection(int *x, int *y, int size_x, int size_y, int opt, bool show = false)
{
  ...
  if (opt == 2)
    std::sort(probe_array, probe_array + probe_size, hash_compare_opt);
  ...
  intersection_kernel<<<BLOCK_NUM, BLOCK_SIZE>>>(d_probe_array, probe_size, d_build_hash_table, hash_table_size, d_output, opt, show);
...
int main(void)
{
  ...
  int size_x = 64, range_x = 4096 * 2;
  int size_y = 4096, range_y = 4096 * 2;
  ...
  warm_up_gpu<<<216, 1024>>>();

  std::tuple<int, int *> result_gpu = intersection(x, y, size_x, size_y, 0, true);
  ...
  std::tuple<int, int *> result_opt = intersection(x, y, size_x, size_y, 1, true);
  ...
  std::tuple<int, int *> result_opt_opt = intersection(x, y, size_x, size_y, 2, true);
  printf("size is : %d\n", std::get<0>(result_opt_opt));
  if (array_are_equal(std::get<1>(result_loop), std::get<1>(result_opt_opt), std::get<0>(result_loop)))
    printf("intersection correct for OPT_version!\n");
